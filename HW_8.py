...
def analyze_packets(packets):
    ...
    for pkt in packets:
        ...
        if not (text.startswith('HTTP/') or 'GET ' in text or 'POST ' in text):
            continue

        # Проверяем gzip
        if 'content-encoding: gzip' in text.lower():
            http_data.append(
                text +
                "\n\n[Тело ответа сжато gzip. "
                "Для корректной распаковки требуется сбор TCP-потока]"
            )
        else:
            http_data.append(text)

    print(f"Найдено HTTP-сообщений: {len(http_data)}")

    for i, msg in enumerate(http_data[:40], 1):
        print(f"\nHTTP-сообщение {i}")
        print(msg[:300])
